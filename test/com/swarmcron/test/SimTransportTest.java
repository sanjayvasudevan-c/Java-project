...
final class SimTransportTest {
    ...
    private static void deliversAfterLatency() {
        ...
        world.network().setDefaultLink(new SimNetwork.LinkConfig(10, 10, 0.0));
    ...
    private static void withholdsBeforeLatency() {
        SimWorld world = new SimWorld(0, 1);
        world.network().setDefaultLink(new SimNetwork.LinkConfig(50, 50, 0.0));
        PeerAddress a = new PeerAddress("sim", 1);
        PeerAddress b = new PeerAddress("sim", 2);
        List<MessageType> received = new ArrayList<>();
        SimTransport ta = new SimTransport(a, world.network());
    ...
    private static void dropsOnFullLoss() {
        ...
        world.network().setDefaultLink(new SimNetwork.LinkConfig(1, 1, 1.0));
        PeerAddress a = new PeerAddress("sim", 1);
        PeerAddress b = new PeerAddress("sim", 2);
        List<MessageType> received = new ArrayList<>();
        SimTransport ta = new SimTransport(a, world.network());
    ...
    private static long runWithSeed(long seed) {
        SimWorld world = new SimWorld(0, seed);
        world.network().setDefaultLink(new SimNetwork.LinkConfig(1, 20, 0.3));
        PeerAddress a = new PeerAddress("sim", 1);
        PeerAddress b = new PeerAddress("sim", 2);
        List<MessageType> received = new ArrayList<>();
        SimTransport ta = new SimTransport(a, world.network());
